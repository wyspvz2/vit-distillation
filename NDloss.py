...
import pos_on_class
def compute_projection_l2_loss(student_features, teacher_features, projected_features_S, projected_features_T, labels, num_classes, device='cuda'):
    """
    计算教师和伪教师投影后的L2损失，并对每个类别求平均，再对所有类别的损失求均值
    Args:
        student_features: 伪教师特征，形状为 [B, 197, 1024]
        projected_features_T: 教师特征，形状为 [B, 197, 1024]
    ...
    for k in range(num_classes):
        ...
        # 计算L2损失
        l2_loss = torch.norm(class_student_features - class_projected_features_T, p=2, dim=-1)  # [N_k, 197]
