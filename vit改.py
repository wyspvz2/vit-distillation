...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            mlp_layer: Type[nn.Module] = Mlp,
            enable_distill_module: bool = True,  # 新增
    ) -> None:
        """
        Args:
        ...
        self.enable_distill_module = enable_distill_module
        if self.enable_distill_module:
    ...
    def forward_features(self, x: torch.Tensor, 
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if (self.enable_distill_module and enable_distill and distill_layer_idx is not None and i == distill_layer_idx and self.distill_module is not None):
                if use_teacher:
                    if teacher_features is None:
                        raise ValueError("teacher_features must be provided when use_teacher=True")
                    teacher_feat = teacher_features[i]
                else:
                    teacher_feat = None  # 推理阶段用伪教师



            

                # 蒸馏模块处理cls和patch tokens
                _, x, _, _, distill_loss = self.distill_module(
                    opt_feat=teacher_feat,
                    sar_feat=x,
    ...
    def forward(self, 
                x: torch.Tensor, 
                teacher_features: Optional[list] = None, 
                opt: torch.Tensor = None,   # 新增opt输入
                batch_idx: int = None,      # 当前batch索引，用于命名
                save_vis_path: str = None,  # 可视化图片保存目录
                attn_mask: Optional[torch.Tensor] = None,
                distill_layer_idx: Optional[int] = None,
                use_teacher: bool = True,               #  新增参数
                enable_distill: bool = True
                ) -> tuple:

        # 1. 可视化保存
        """if opt is not None and save_vis_path is not None and batch_idx is not None:
            os.makedirs(save_vis_path, exist_ok=True)

            vis_num = min(4, x.size(0), opt.size(0))  # 最多保存4张
            vis_sar = x[:vis_num]
            vis_opt = opt[:vis_num]

            if vis_sar.size(1) == 1:
                vis_sar = vis_sar.repeat(1, 3, 1, 1)
            if vis_opt.size(1) == 1:
                vis_opt = vis_opt.repeat(1, 3, 1, 1)

            vis_pair = torch.cat([vis_opt, vis_sar], dim=3)

            vutils.save_image(
                vis_pair,
                os.path.join(save_vis_path, f'opt_sar_pair_batch_{batch_idx}.png'),
                normalize=True, scale_each=True, nrow=1
            )
        """  

        # 2. 正常前向传播（带蒸馏控制）
        x, features, distill_loss = self.forward_features(
            x,
            attn_mask=attn_mask,
            teacher_features=teacher_features,
            distill_layer_idx=distill_layer_idx,
            use_teacher=use_teacher,
            enable_distill=enable_distill       # ✅ 加入控制开关
        )
